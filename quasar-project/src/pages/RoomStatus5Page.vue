  ...
  <div class="fullscreen" :style="{ backgroundColor: roomStatus?.color || '#fff' }" @click="toggleMenu">
    <div class="header" :style="{ backgroundColor: headerColor }">
      <div class="row items-center">
        ...
        <div class="col">
          <span class="mc-room-number pull-right">05</span>
        </div>
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    try {
      const roomResponse = await axios.get('http://localhost:3001/api/rooms/room-5');
  ...
  methods: {
    ...
    async changeStatus(status) {
      try {
        ...
        await axios.put(`http://localhost:3001/api/rooms/room-5`, { status });
